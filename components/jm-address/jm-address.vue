	...
	export default {
		...
		methods: {
			...
			// 
						...
						// 	var that = this;
						// 	uni.request({
						// 		url: that.$api+'default/district&access_token='+that.$access_token, //仅为示例，并非真实接口地址。
						// 		data: {
						// 			pid: id
						// 		},
							...
							case 3:
								...
								for (var i = 0; i < res.data.data.length; i++) {
									for(var j = 0;j < res.data.data[i].list.length;j++){
										if (res.data.data[i].list[j].id == id) {
											newArr2 = res.data.data[i].list[j].list
										}
									}
								}
								_self.addressd3 = newArr2;
								_self.csname = newArr2[0].name;
								_self.cs = newArr2[0].id;
								break;
							default:
							return;
						}
