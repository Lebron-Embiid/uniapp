...
<template name="glanceShopcart">
	...
	<view class="glance-shop-cart-order" v-if="!shownullcart">
		...
		<!-- 总数量 -->
		<view class="glance-shop-cart-total-cnt">全选</view>
		<!-- 总金额 -->
		<view class="glance-shop-cart-total-amt">总计：￥{{ totalamount }}</view>
		<!-- 立即下单 -->
					 ...
					 for (let i = 0; i < s_index.length; i++) { 
						uni.request({
							...
							success: res => { 
								 if(res.data.code == 0){
									 var ss_list = [];
								    console.log(s_cart.length)
									 for (let j = 0; j < s_cart.length; j++) { 
										  ss_list.push({
											  cart_id:s_cart[j]
										  })	
										}		
...
<style lang="scss">
	...
	.glance-shop-cart-items-item-opt{
		...
		width: 40%;height: 100%; font-size: 20px;text-align: center;
	...
	.glance-shop-cart-order{
		width: 100%;height: 95upx;background-color: #ffffff;
		display: flex;display: -webkit-flex;flex-flow: row nowrap;align-items: center;justify-content: space-between;
		position: fixed;
		bottom: 0px;
	...
	// 合计金额样式
	.glance-shop-cart-total-cnt{width:10%;text-align: left;line-height:40px;font-size: 13px;}
	
	// 合计金额样式
	.glance-shop-cart-total-amt{width:26%;text-align: left; line-height:40px;font-size: 24upx;color: #fa3930;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;}
	
	// 生成订单样式
	.glance-shop-cart-create-order{width: 50%;background-color: #fa3930;text-align: center;line-height:95upx;font-size: 28upx;color: #ffffff;}
